...
- **La vidéo doit peser moins de 300 Mo** — contrainte relue le 29/08 sur la page lablab "Hackathon Guidelines", au mot près : *"Provide a link to your video presentation (ensure it's under 300MB and within 5 minutes duration)"*. Les 5 minutes étaient déjà notées ici ; **la limite de taille, non**. Un enregistrement d'écran non compressé (ProRes, capture brute) la dépasse sans difficulté, alors qu'un H.264 1080p de 5 minutes passe largement — donc **vérifier la taille du fichier avant de le téléverser**, pas après. La même page dit "provide a link", pas "upload" : préparer les deux cas.
- **Cover image de la soumission** : **faite** le 29/08 — `submission/cover.png`, 1920×1080 (16:9), régénérable par `submission/faire_cover.py`. Pillow n'est délibérément **pas** dans `requirements.txt` (l'agent n'en a aucun besoin) : l'installer à côté, `python3 -m pip install --target /tmp/pylibs Pillow`, puis `PYTHONPATH=/tmp/pylibs python3 submission/faire_cover.py`.
- **Réponse prête si un juge demande "est-ce que ça existe déjà" (recherché 25/08)** : le mécanisme s'apparente à la Probability of Backtest Overfitting / walk-forward optimization de la littérature quant (López de Prado et al.), mais validées une fois à la conception — la vraie différence de ce projet, c'est que le test tourne à CHAQUE décision live, pas une seule fois. Détail complet dans `DESIGN_NOTES.md` ("Where this sits in the existing literature") — descendu du README le 30/08, quand celui-ci est passé de 5 948 à 2 525 mots, pas la peine de le réciter dans la vidéo elle-même — juste avoir la réponse prête pour un Q&A.
