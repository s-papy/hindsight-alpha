...
- **Le débit change tout, donc chronométrer une répétition avant de tourner.** À 145 mots/min → 4:31. À 165 (soutenu) → 4:09. À 130 (lent, très articulé) → 4:48, soit 12 secondes de marge seulement. En dessous de ~125 mots/min le script dépasse : si la première répétition sort au-delà de 4:45, couper un bloc plutôt que d'accélérer le débit. Le guide lablab insiste aussi sur "démo tôt" — ici le tableau de bord est à l'écran dès la première seconde, et le premier run réel à 1:12.
